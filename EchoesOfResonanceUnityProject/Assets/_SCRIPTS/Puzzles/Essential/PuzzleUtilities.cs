...
public struct TrData
{
    ...
    public IEnumerator ApplyToOverTime(Transform tr, float duration, AnimationCurve curve = null, bool useGlobalPosition = false)
    {
        if (curve == null)
        {
        ...
        Quaternion startRot = useGlobalPosition ? tr.rotation : tr.localRotation;
        ...
        while (elapsed < duration)
        {
            ...
            float evalT = curve.Evaluate(t);

            if (useGlobalPosition)
            {
                tr.position = Vector3.Lerp(startPos, targetPos, evalT);
                tr.rotation = Quaternion.Lerp(startRot, targetRot, evalT);
            }
            else
            {
                tr.localPosition = Vector3.Lerp(startPos, targetPos, evalT);
                tr.localRotation = Quaternion.Lerp(startRot, targetRot, evalT);
            }

            if (scale != Vector3.zero)
                tr.localScale = Vector3.Lerp(startScale, targetScale, evalT);
