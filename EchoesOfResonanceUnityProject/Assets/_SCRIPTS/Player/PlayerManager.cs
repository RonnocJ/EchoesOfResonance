...
public class PlrMngr : Singleton<PlrMngr>, IInputScript, ISaveData
{
    ...
    private float currentLookInput;
    ...
    void Update()
    {
        currentLookInput = Mathf.Lerp(currentLookInput, lookInput, Time.deltaTime * 7.5f);

        if (Mathf.Abs(currentLookInput) > 0.025f && GameManager.root.State >= GameState.InPuzzle)
        {
            transform.localEulerAngles += Vector3.up * currentLookInput * lookSpeed * Time.deltaTime;

            if (GameManager.root.State == GameState.InPuzzle)
            {
                Broadcaster.activePlate.standTr.localRotation = Quaternion.Euler(new Vector3(0, transform.localEulerAngles.y, 0));
                
                if(Broadcaster.activePlate is MultiPuzzleStand multiPlate) multiPlate.CheckRotation(transform.localEulerAngles.y);
            }
        }
