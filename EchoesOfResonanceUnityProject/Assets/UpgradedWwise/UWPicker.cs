...
public class UWPicker : EditorWindow
{
    ...
    protected virtual void CreateGUI()
    {
        ...
        if (RefreshTreeView(enumData) == 5)
        {
            ...
            {
                var console = AkUtilities.GetWwiseConsole();
    ...
    where T : AkWwiseProjectData.AkInformation
    {
        ...
        {
            string workUnitName = workUnit.ParentPath.Replace($"{rootCategory}/Default Work Unit/", string.Empty);
            int depth = workUnitName.Count(c => c == '/');
            workUnitName = GetLastName(workUnitName);
            ...
            workUnitMap[workUnitName] = (workUnitNode, (depth == 0) ? true : false);
        }

        foreach (var workUnit in sortedWorkUnits)
        {
            string workUnitPath = workUnit.ParentPath.Replace($"{rootCategory}/Default Work Unit/", string.Empty);
            string workUnitName = GetLastName(workUnitPath);
            string parentName = GetParentName(workUnitPath);

            if (parentName != null && workUnitMap.TryGetValue(parentName, out var parentNode))
            {
                var updatedChildren = parentNode.data.children.ToList();
                updatedChildren.Add(workUnitMap[workUnitName].data);
                var updatedParentNode = new TreeViewItemData<string>(parentNode.data.id, parentNode.data.data, updatedChildren);

                workUnitMap[parentName] = (updatedParentNode, workUnitMap[parentName].topLevel);
            }
        }
