...
public class AudioEnumGenerator : MonoBehaviour
{
    ...
    {
        var jsonData = File.ReadAllText("Assets/StreamingAssets/Audio/GeneratedSoundBanks/Windows/SoundbanksInfo.json");
        var enumCodeSections = ExtractEventNamesFromJson(jsonData);

        var fileContents = GenerateFileTextForCodeSections(enumCodeSections);
    ...
    static List<EnumCodeSection> ExtractEventNamesFromJson(string jsonData)
    {
        ...
        foreach (var soundBank in soundBanksInfo.SoundBanksInfo.SoundBanks)
        {
            ...
            if (soundBank.StateGroups != null && !hasGeneratedStates)
            {
                for (int i = 0; i < soundBank.StateGroups.Count; i++)
                {
                    for (int j = 0; j < soundBank.StateGroups[i].States.Count; j++)
                    {
                        if (!stateNames.Contains(soundBank.StateGroups[i].States[j].Name))
                        {
...
";
    ...
    {
        var directory = Application.dataPath + "/_SCRIPTS/Audio/_generated/";
