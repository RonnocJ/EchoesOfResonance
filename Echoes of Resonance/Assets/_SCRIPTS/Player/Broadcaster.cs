...
public class Broadcaster : Singleton<Broadcaster>, IInputScript
{
    ...
    void SetLastNote(float newNote)
    {
        ...
        CRManager.root.Begin(DrainBatteryRoutine(noteSustainDrainAmount * notesHeld), "DrainBatteryNote", this);
    }
    ...
    public void RemoveNote(float oldNote)
    {
        ...
        if (notesHeld == 0)
        {
            ...
            CRManager.root.Begin(RegenRoutine(), "Regen", this);
    ...
    void Update()
    {
        batteryMeter.fillAmount = Mathf.SmoothStep(batteryMeter.fillAmount, _batteryLevel, Time.deltaTime * 5f);
        ...
        if (GameManager.root.currentState == GameState.Roaming && gemLaser.enabled)
        {
            AdjustModValue(0);
        }
    }
