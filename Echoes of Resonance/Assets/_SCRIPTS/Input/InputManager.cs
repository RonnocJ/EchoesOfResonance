...
{
    [HideInInspector]
    public bool usingMidiKeyboard, settingsToggleState;
    ...
    private Dictionary<Key, float> keyNoteMapping = new Dictionary<Key, float>
{
    ...
    void InitializeActions()
    {
        ...
        else
        {
            ...
            map.FindAction("Settings").performed += _ =>
            {
    ...
    {
        var attribute = (AllowedStatesAttribute)Attribute.GetCustomAttribute(method.Method, typeof(AllowedStatesAttribute));
        ...
        if (attribute != null)
        {
            wrappedMethod = input =>
            {
                if (Array.Exists(attribute.States, state => state == GameManager.root.currentState))
                {
                    method(input);
                }
            };
    ...
    void OnDisable()
    {
        ...
        if (GameManager.root.currentState != GameState.Config)
        {
            ...
            else
            {
                ...
                map.FindAction("KeyDown").performed -= _ => checkingKeys = true;
                map.FindAction("KeyUp").performed -= _ => checkingKeys = false;

                map.FindAction("ArrowVertical").performed -= amount => actionDict[ActionTypes.ModwheelChange].actionEvent.Invoke(amount.ReadValue<float>());
                map.FindAction("ArrowVertical").canceled -= _ => actionDict[ActionTypes.ModwheelChange].actionEvent.Invoke(0);

                map.FindAction("ArrowHorizontal").performed -= amount => actionDict[ActionTypes.ModwheelChange].actionEvent.Invoke(amount.ReadValue<float>());
                map.FindAction("ArrowHorizontal").canceled -= _ => actionDict[ActionTypes.PitchbendChange].actionEvent.Invoke(0);
            }
        }
    }
