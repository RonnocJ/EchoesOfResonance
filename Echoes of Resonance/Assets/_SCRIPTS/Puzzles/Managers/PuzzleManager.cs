...
public class PuzzleManager : Singleton<PuzzleManager>, IInputScript
{
    ...
    void Start()
    {
        var allPuzzleGUIDs = AssetDatabase.FindAssets($"t:{typeof(PuzzleData).Name}");
        var allPuzzles = allPuzzleGUIDs.Select(guid => AssetDatabase.LoadAssetAtPath<PuzzleData>(AssetDatabase.GUIDToAssetPath(guid))).ToArray();
